...
contract('Dex', (accounts) => {
  ...
  it('should NOT withdraw tokens if token does not exsit', async()=>{
     await expectRevert(
       dex.withdraw(web3.utils.toWei('100'), web3.utils.fromAscii('TOKEN-DOES-NOT-EXSIT'), 
       {from: trader1}),
       'this token does not exist'
  );
     });
  it('should NOT withdraw tokens if balance is too low', async()=>{
   ...
   await expectRevert(dex.withdraw(web3.utils.toWei('1000'), DAI, {from:trader1}),
   'balance too low' );
  });
  ...
  it('should not create limit order if tokens balance is low', async ()=> {
    await dex.deposit(web3.utils.toWei('99'), REP, {from:trader1});
    await expectRevert(dex.createLimitOrder(REP, web3.utils.toWei('100'), 10, SIDE.SELL, {from: trader1}),
      'token balance too low');
    });
    it('should not create limit order if DAI balance low', async()=>{
